...
├── config/
│   ├── GlobalExceptionAdvice.java          # 全局异常处理（Graceful Response）
│   ├── JdbcConfig.java                     # JDBC 配置类（DataSource、NamedParameterJdbcTemplate）
│   └── SqlCaptureExecutor.java             # SQL 捕获与执行封装（泛型 record）
├── controller/
│   └── UserController.java                 # Controller 层
├── dao/
│   └── UserDao.java                        # DAO 层（SQL 拼接与执行函数注入）
├── dto/
│   ├── User.java                           # 用户实体
